...
export default function CoursesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F8FF]">
      <section className="bg-[#F0F8FF] pt-24 pb-12 px-6">
        <div className="max-w-7xl mx-auto text-center">
          <h1
            className={`text-4xl md:text-5xl font-bold text-[#1d588a] mb-4 transition-all duration-1000 ${
          ...
          <p
            className={`text-lg md:text-xl text-gray-600 mb-8 max-w-3xl mx-auto leading-relaxed transition-all duration-1000 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
            data-aos="fade-up"
            data-aos-delay="200"
          ...
          <div
            className={`flex flex-col md:flex-row justify-center items-center gap-8 text-base transition-all duration-1000 delay-400 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
            data-aos="fade-up"
            data-aos-delay="400"
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {courses.map((course, index) => (
              <Card
                key={course.id}
                className="bg-white/90 backdrop-blur-sm shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] border-0 h-full flex flex-col"
                data-aos="fade-up"
                data-aos-delay={index * 100}
              >
                <CardHeader className="pb-4">
                  <div className="flex items-start justify-between mb-3">
                    <Badge className="bg-[#1d588a]/10 text-[#1d588a] hover:bg-[#1d588a]/20 text-xs px-2 py-1">
                  ...
                  <div className="flex gap-3 w-full">
                    <Link href={`/courses/${course.id}`} className="flex-1">
                      <Button
                        size="sm"
                        variant="outline"
                        className="w-full text-sm py-2 rounded-lg hover:scale-105 transition-all duration-300 bg-transparent border-[#1d588a] text-[#1d588a] hover:bg-[#1d588a] hover:text-white"
                      >
                        View Details
                      </Button>
                    </Link>
                    <Link href={`/payment?course=${course.id}`} className="flex-1">
                      <Button
                        size="sm"
                        className="w-full text-sm py-2 rounded-lg hover:scale-105 transition-all duration-300 bg-[#1d588a] hover:bg-[#1d588a]/90"
                      >
                        Enroll Now
                      </Button>
                    </Link>
