...
export async function POST(req: Request) {
  try {
    ...
    if (!file) {
      return NextResponse.json({ success: false, message: "Profile picture is required" }, { status: 400 });
    }
    ...
    if (!allowedTypes.includes(file.type)) {
      return NextResponse.json({ 
        success: false, 
        message: "Invalid file type. Only JPEG, JPG, PNG, WEBP, and GIF are allowed." 
      }, { status: 400 });
    }
